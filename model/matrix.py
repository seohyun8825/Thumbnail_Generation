...
def keyconverter(self, split_ratio, usebase):
    ...
    print(f"[DEBUG] Initial split_ratio: {split_ratio}")
    if isinstance(split_ratio, list):
        # 리스트를 문자열로 변환
        split_ratio = ";".join(",".join(map(str, row)) for row in split_ratio)    
    try:
        ...
        print(f"[ERROR] split_l2 failed with error: {e}")
        raise

    print(f"[DEBUG] split_ratio2 before ratiosdealer: {split_ratio2}")
    print(f"[DEBUG] split_ratio2r before ratiosdealer: {split_ratio2r}")

    try:
        split_ratio2, split_ratio2r = ratiosdealer(split_ratio2, split_ratio2r)
    except Exception as e:
        print(f"[ERROR] ratiosdealer failed with error: {e}")
        raise
